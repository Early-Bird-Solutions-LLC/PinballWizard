...
public sealed class ScraperSettings
{
    ...
    /// <summary>OPDB API token for authenticated export endpoint. Set via config or OPDB_API_TOKEN env var.</summary>
    public string? OpdbApiToken { get; set; }

    /// <summary>IFPA API key (WPPRKey) for player rankings and tournament data. Set via config or IFPA_API_KEY env var.</summary>

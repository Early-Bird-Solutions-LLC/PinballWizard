...
//
// Implements IMachineDocumentReadRepository by extending CosmosRepository<ScrapedDocumentRecord>
...
//
// Enrichment: each ScrapedDocumentRecord carries the scraped-side fields (DocumentId, DocumentType,
// DocumentUrl, Edition, EditionScope, LastDownloadedAt). The raw-side fields (LinkText, LinkStatus,
// ResolutionStrategy, SizeBytes, PageCount) live in the `scraped_documents_raw` container and are
// fetched per-document via IRawDocumentRepository.GetAsync. The raw doc may be null for documents
// that have not yet been processed by the linker — enrichment fields null-propagate in that case.
internal sealed class CosmosMachineDocumentReadRepository
    : CosmosRepository<ScrapedDocumentRecord>, IMachineDocumentReadRepository
{
    ...
    public CosmosMachineDocumentReadRepository(
        ...
        ILogger<CosmosRepository<ScrapedDocumentRecord>> logger)

...
- [ ] ADRs 0019, 0020, 0021, 0022, 0023 committed; [`docs/adr/README.md`](adr/README.md) indexes them; `CLAUDE.md` and `docs/guardrails.md` § Locked decisions reference the relevant ADRs (no inline duplicates of the rationale)
- [ ] `data/phase4/curated-subset.v1.json` slate manifest committed; `tools/phase4/VerifyCuratedSubsetCoverage.csx` confirms each machine has ≥1 PDF in deployed `scraped_documents`
- [ ] All four inherited Phase 3 follow-ups closed: connected-agents wired (item 7), tool-trace citation extraction (item 9), eval ground-truth re-curated (item 8), `SubAgentUsed` reads from Foundry trace (item 10). Item 11 (`NullTokenUsageReader` real impl) tracked; closure is conditional on agent-framework#2688
...
- [ ] Curated 7-machine subset successfully indexed end-to-end: each machine has metadata-card chunks + manual chunks (≥1 each) in the deployed `pinwiz-rag-v1` index; index document count matches the expected total within ±5% (allowing for chunking variance)
...
- **Service-bulletin chunking and indexing** — Phase 4.5 or later (the index schema in ADR-0021 supports `document_type=service_bulletin`, but Phase 4 only populates `manual` and `metadata_card`)
...
The 24 scope items split into a doc-only Wave 0, a parallel Wave 1 spanning A-track and B-track foundations, a Wave 2 that combines A-track completion with B-track core ingestion, two retrieval-focused waves, and a closing eval + retrospective wave. Three operational hand-offs (H1 / H2 / H3) punctuate the sequence. Net: ~14–17 PRs + 3 hand-offs across 6 waves.

#### Dependency core (sequential)

`(W0) ADRs 0019–0023 → (W1) item 7 → items 9 + 10 → (W1) H1 → (W1) item 13 → (W2) item 14 → item 15 → items 16/17 → (W3) item 19 → (W4) item 20 → items 21/22 → H3 → (W5) item 24`

- **W0 → W1** — ADRs lock the design decisions; A-track items 7–10 reference them; B-track item 12 (Bicep flip) references ADR-0021's index-schema design.
- **Item 7 → items 9 + 10** — connected-agents wiring is the structural prerequisite for tool-trace citation extraction and for reading `SubAgentUsed` from the trace.
- **H1 → item 15** — embedding pipeline + AI Search index population needs the AI Search service to exist.
- **Item 13 → item 14 → item 15 → items 16/17** — extraction → chunking → indexing → orchestrator (Change Feed Function + metadata-card synthesis).
- **Items 15/16/17 → item 19** — query client integration tests need a populated index.
- **Items 19 + 9 → item 20** — Wizard retrieval integration combines retrieval with citation extraction.
- **Items 20 → 21/22** — citation coverage and citation-required guardrail wrap the retrieval pipeline.
- **Item 22 → H3** — final eval needs all retrieval + guardrail surfaces in place.
- **H3 → item 24** — observability + locked-decisions + retrospective references calibrated values.

#### Independent surface (parallel-safe)

- **Item 8 (eval re-curation)** is independent of every other Phase 4 item — script + data-file change. Worktree-isolated `general-purpose` subagent dispatch (matches Phase 3 PR 8 pattern).
- **Item 12 (Bicep flip)** has no code-file conflict with A-track — different namespace, different repo area. Can run concurrent with items 7+ except H1 hand-off must precede item 15's merge.
- **Item 19 (AI Search query client)** can be coded in parallel with item 15 by mocking the index for unit tests; integration tests gate on item 17 landing.
- **Items 21 + 22** can run in parallel — citation-coverage is a new evaluator (touches Application/Ai/Evaluation/), citation-required guardrail extends `RefusalCategory` and `AiRouter` (touches Application/Ai/). Some `AiRouter.cs` conflict but small.

#### Recommended waves (respecting `guardrails.md` § Parallelism ceiling 2–3 PRs in flight)

**Wave 0** (1–3 PRs, doc-only; sequential ahead of code) — drafts Phase 4 § Scope (this PR), batches ADRs 0019–0023 (one PR), optionally enhances `guardrails.md` § Per-phase gate with a README-claim review item (W0-3, lesson from PR #94).

**Wave 1** (4–5 PRs in flight) — A-track inherited follow-ups + B-track foundations:

- **PR W1-1 (item 7, A1)** — Connected-agents wiring on Wizard.
- **PR W1-2 (item 9, A2)** — Tool-call-trace citation extraction (sequenced after W1-1 file conflict on `AiRouter.cs`).
- **PR W1-3 (item 8, A5)** — Eval re-curation (worktree-isolated subagent; no code-file conflict).
- **PR W1-4 (item 12, B2)** — Bicep `deployAiSearch=true` flip (1-line param change).
- **PR W1-5 (item 13, B3)** — PdfPig wrapper + text-extraction service.

After Wave 1: deploy hand-off **H1** (Bicep apply + AI Search smoke probe).

**Wave 2** (3–4 PRs in flight + 1 hand-off) — A-track completion + B-track core ingestion + intermediate eval:

- **PR W2-1 (item 10, A3)** — Read `SubAgentUsed` from Foundry trace (sequenced after W1-1; small change).
- **PR W2-2 (item 14, B4)** — Hybrid chunker.
- **PR W2-3 (item 15, B5)** — Embedding pipeline + AI Search index population (sequenced after H1 + W2-2 file conflicts).
- **PR W2-4 (item 6)** — Curated subset slate manifest + verification script (can land any time in Wave 1 or 2; sequenced into Wave 2 to keep Wave 1 focused on inherited follow-ups).
...
- **PR W3-1 (item 16, B7)** — Metadata-card synthesis.
- **PR W3-2 (item 17, B6)** — Cosmos Change Feed Function (sequenced after W3-1 file conflicts on the Functions project).
- **PR W3-3 (item 19, C1)** — AI Search query client + hybrid retrieval (parallel via mocked index).

**Wave 4** (3 PRs in flight) — Wizard retrieval integration + guardrail:

- **PR W4-1 (item 20, C2)** — Wizard retrieval integration (`searchCorpus` tool).
- **PR W4-2 (item 21, C3)** — Citation-coverage measurement.
- **PR W4-3 (item 22, C4)** — Citation-required guardrail (sequenced after W4-1 file conflicts).

After Wave 4: operational hand-off **H3** (final eval baseline + threshold calibration).

**Wave 5** (1 PR) — closeout:

- **PR W5-1 (item 24)** — observability.md + guardrails.md § Locked decisions + Phase 4 § Retrospective + README/vision.md per-phase-close review.
...
| Wave 5 | 1 (closeout) | 0 | n/a |
| **Total** | **~15–19 PRs** | **3 hand-offs** | |

#### Conventions for this phase

- **The ADR batch PR (Wave 0)** still gets explicit user confirmation per `guardrails.md` § Decision framework before commit; ADRs are append-only and a wrong one is expensive to reverse. Each of the 5 ADRs may split into its own PR if scope-sized.
...
- **Worktree-isolated `general-purpose` subagents** are the canonical pattern for items 8 (eval re-curation) and the Wave 0 ADRs — independent surfaces with no main-context dependencies.
...
- **Live integration tests gated by `PINBALL_WIZARD_LIVE_RAG_TESTS=1`** are required for items 17 (Change Feed Function) and 19 (query client) — DL-0002 + DL-0003 lessons honored. CI runs them via a manually-triggered workflow, not on every push.
...
| P4-R4 | PdfPig fails on a curated-subset PDF (encrypted, scanned, or complex layout breaks extraction) | `ExtractionStatus.OcrRequired` path logs and skips; if a curated-subset machine fails extraction, swap for the documented alternate per item 6's slot. Phase 4.5 owns the OCR-fallback decision so Phase 4 isn't blocked by long-tail edge cases |
...
6. **Whether service-bulletin records also get chunked + indexed in Phase 4** (the index schema supports `document_type=service_bulletin`, but Phase 4 § Scope only includes `manual` + `metadata_card`). Default-no; revisit in Phase 4.5. If user-traffic queries surface repair questions where bulletins would obviously help, consider folding bulletin-chunking into Phase 4 mid-flight.
...
| Service-bulletin chunking and indexing | Phase 4 architecture lock (2026-05-07) | Index schema supports `document_type=service_bulletin`; Phase 4 only populates `manual` + `metadata_card` to keep scope bounded | Phase 4.5 evaluates folding bulletins in alongside manuals expansion |

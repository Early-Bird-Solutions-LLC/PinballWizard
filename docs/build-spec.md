...
| 2 | Runtime validation — `ingestion_sources` seeded, OPDB sync against deployed Cosmos, Phase 2 Bicep gating decisions, operational metrics groundwork | 🟡 In progress |
...
6. **Playwright 1.49+ upgrade.** Bump `Microsoft.Playwright` from 1.12.0 to current stable (≥ 1.49). Validate Stern scrapers (`ManualsScraper`, `GamePageScraper`, `ServiceBulletinScraper`) still parse against the live site. Remove the `LinkRaw` / `EditionRaw` / `BulletinRaw` records-workaround from PR #34 (the `Activator.CreateInstance` path that motivated the conversion to classes with init-able properties + `[JsonPropertyName]`); re-introduce records *only if* the upgraded Playwright still requires the workaround. If the workaround is still needed post-upgrade, document the reason in a comment and skip the records-revert.

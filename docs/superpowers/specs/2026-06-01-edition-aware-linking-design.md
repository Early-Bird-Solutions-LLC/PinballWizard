...
2. **Group-aware multi-match.** When the normalized title matches **multiple** base machines that **share a `GroupId`**, this is NOT ambiguous — it is the expected edition-family case. Write `ManufacturerSlugs[mfr] = game.Slug` to **every** base in the matched group (closes D4 — 22 ambiguous). Merge `game.Editions` into each.
3. **True ambiguity preserved.** If the multi-match machines do NOT share a `GroupId` (genuinely unrelated machines colliding on title), keep the existing `Ambiguous` outcome and **log both candidate OPDB ids** (no silent drop — gate G1).
4. New `MatchOutcome.Group` for telemetry; reconciliation result counts group-matched separately.

The reconciler does **not** need an OPDB id on `GameRecord` after all — the catalog already carries `GroupId` and per-edition `Title`/`features`, which is sufficient to populate slugs across the group. (Simpler than the ADR-0031 "add OpdbId to GameRecord" sketch; the group-slug approach subsumes it. ADR-0031 to be amended to reflect this.)

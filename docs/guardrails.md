...
- Per-`AIAgent` model selection: gpt-4o default for Wizard / Valuation / Rules; gpt-4.1 for Repair + escalation tier. In-process LRU semantic cache (~512 entries, prompt-version-keyed). Per-call cost ceiling enforced. ([ADR-0015](adr/0015-cost-routing-and-semantic-cache.md))
...
- **Machine findability routes through Azure AI Search** — `getMachineByTitle` uses a machine catalog AI Search index for fuzzy / prefix / phonetic / synonym / ranked disambiguation, with the Cosmos exact point-read kept as the fast path. Relevance ranking uses content-intrinsic scoring profiles (freshness, completeness-magnitude, canonicality-tag) — no engagement or popularity signals. Stays on AI Search Basic; revisit S1 only on corpus or concurrency growth past Basic limits. ([ADR-0049](adr/0049-findability-and-relevance-ranking-program.md), Accepted 2026-07-02)

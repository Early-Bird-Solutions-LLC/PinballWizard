...
| `EditionInfo.Availability` | [GameRecord.cs:33](../src/PinballWizard.Scraper/Models/GameRecord.cs#L33) | **partial** ✓ | `sold_out` populated via `contact-for-availability` URL parsing ([StaticMetadataExtractor.cs](../src/PinballWizard.Scraper/Scrapers/StaticMetadataExtractor.cs)). **2026-08:** Stern replaced those links with relative `/contact-to-buy` links that carry no availability marker, so `sold_out` is no longer populated for current Stern pages. `vault`/`archive` still pending — derive from `GameRecord.DiscoveredOn` |
...
| `EditionInfo.Msrp` | [GameRecord.cs:32](../src/PinballWizard.Scraper/Models/GameRecord.cs#L32) | **wired (legacy links only)** | Populated from `contact-for-availability` URL `price` query param. **2026-08:** Stern's replacement `/contact-to-buy` links do not encode `price`, so MSRP is null for current Stern pages; the legacy parse is retained for pages still using the old links |

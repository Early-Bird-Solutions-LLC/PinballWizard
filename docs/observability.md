...
| `pinwiz.scraper.yield_guard_failures_total` | Counter | `scraper`, `guard` | Runs that failed either per-scraper yield guard. **Two** guards increment this counter, keyed on `<ISourceScraper.Name>`, each defaulting to **1** when its key is absent — write an explicit `0` to opt out; a negative value disables as `0` does but logs a Warning. **(1) Item guard** (`Scraper:MinimumYieldPerScraper`) — "did this scraper collect anything at all?" An item is a document link **or** a game record, one carrying both counting once, so the four catalogue-only scrapers (JJP, Pinball Brothers, Multimorphic, Barrels of Fun) are measured on the machines they collect rather than on links they never emit. **(2) Link guard** (`Scraper:MinimumLinkYieldPerScraper`) — "did link extraction still work?" The mixed-shape scrapers (Stern Game Pages, Chicago Gaming, Spooky Pinball) emit game records and links as *separate* items, so on the total alone a run whose PDF extraction broke completely would be carried over the minimum by its game records and pass silently. The catalogue-only four carry an explicit `0` here in `appsettings.json`; that is safe only because guard 1 still covers them. The link guard is a refinement, not a peer: it is skipped when guard 1 already fired (one condition, one error) and when guard 1 is opted out entirely. Read a catalogue-only scraper's health from the **absence** of an increment here — their `links_discovered_total` is permanently `0` by design, and that zero is a correct baseline, not a fault. The guard appends to `ScrapeResult.Errors` and marks the source run failed, so the CLI exits 1. A non-zero rate means a scraper completed without an unhandled exception yet collected nothing, or collected machines but no documents — the silent-green-job class (invariant #17 / OBS-01). The `guard` tag is `items` (collected nothing at all) or `links` (collected machines but no documents), so the two conditions stay separable on a dashboard without opening the logs. Paired with an Error log naming the scraper, the unit measured, the actual count, and the minimum. |
...
| `pinwiz.download.permanent_rejection_skip_total` | Counter | `source_type` | Documents permanently skipped because the origin returned HTTP 403 Forbidden, 404 Not Found, or 410 Gone. A non-zero steady-state rate is expected for access-controlled or removed files (e.g. Spooky S3 `.pkg` files whose S3 bucket 403s all access, #839); a spike in a new `source_type` means a previously-healthy origin is now rejecting requests. These are terminal skips — reported as `skipped_permanent_rejection` and excluded from the `failed` count, so they do not set a non-zero exit code. Pair with the `Permanently rejected` log line to identify specific documents. Use `--force-redownload` for an operator-driven re-check after the origin is restored. |

...
- **Test project naming:** one `<ProductionProject>.Tests` project per production project (ADR-0030). Integration and end-to-end tests currently live as `[Trait("Category", …)]`-tagged suites *within* those projects — `Category=E2E` in [`Web.Tests/E2E/`](../tests/PinballWizard.Web.Tests/E2E/), integration alongside the relevant `.Tests` — and CI selects them by category, not by a separate project. The `.IntegrationTests` / `.E2ETests` suffixes are the convention to adopt only if/when a suite is split into its own project. See [ADR-0030](adr/0030-test-project-naming.md) (incl. the 2026-06-24 amendment) for the full convention and the project mapping table.

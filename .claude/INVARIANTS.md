...
15. **Community-resource posture.** → standard pending (wave-2 community-posture). PinballWizard routes users outward — outbound traffic is a feature, never editorialized; no engagement-metric framing, no captive UI patterns. ([ADR-0027](../docs/adr/0027-community-resource-posture.md))
16. **Deployment Stacks only.** → `DLV-02` ([delivery](standards/delivery/STANDARD.md)); full two-tier Bicep → standard pending (wave-2 iac-deploy). ([ADR-0013](../docs/adr/0013-two-tier-bicep-deploy.md))

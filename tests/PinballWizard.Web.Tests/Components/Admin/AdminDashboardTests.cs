...
//
// Static SSR + [StreamRendering] (ADR-0034). The six cards load from four
// repositories in OnInitializedAsync. Tests assert the real counts render
// (Machines/Documents/Manufacturers from catalog_stats, Sources, Link Overrides,
// Documents Indexed from IRagCorpusStatsReader) and that a throwing repo
// surfaces the per-card error sentinel (Invariant #17) rather than a silent dash.
public sealed class AdminDashboardTests : AsyncBunitContext
{
    ...
    // Totals: 2 manufacturers, 3 machines, 3 documents.
...
#pragma warning restore CS0162
        ...
        long corpusChunks = 50)
    {
        ...
        var corpus = Substitute.For<IRagCorpusStatsReader>();
        corpus.GetCorpusStatsAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new RagCorpusStats(corpusChunks, [], null)));
    ...
    {
        RegisterAll(StatsStream, sourceCount: 2, overrideCount: 1, corpusChunks: 50);
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            Assert.Equal("2", cut.Find("[data-testid='admin-manufacturers-count']").TextContent.Trim());
            Assert.Equal("50", cut.Find("[data-testid='admin-corpus-chunks-count']").TextContent.Trim());
        });
    }
    ...
    public void StatsLoadFailure_RendersErrorSentinels_NotADash()
    {
        ...
        cut.WaitForAssertion(() =>
        {
            // Machines + Documents + Manufacturers share the catalog_stats load → all error.
            cut.Find("[data-testid='admin-machines-count-error']");
            cut.Find("[data-testid='admin-documents-count-error']");
            cut.Find("[data-testid='admin-manufacturers-count-error']");
    ...
    {
        // Sources stream throws; stats + overrides + corpus are on their happy fakes.
        var stats = Substitute.For<ICatalogStatsReadRepository>();
        stats.StreamAllManufacturersAsync(Arg.Any<CancellationToken>())
            .Returns(ci => StatsStream(ci.Arg<CancellationToken>()));
        Services.AddSingleton(stats);
        ...
        var corpus = Substitute.For<IRagCorpusStatsReader>();
        corpus.GetCorpusStatsAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new RagCorpusStats(75, [], null)));
        Services.AddSingleton(corpus);
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            // Independent loads (stats + overrides + corpus) are unaffected.
            Assert.Equal("3", cut.Find("[data-testid='admin-machines-count']").TextContent.Trim());
            Assert.Equal("1", cut.Find("[data-testid='admin-link-overrides-count']").TextContent.Trim());
            Assert.Equal("75", cut.Find("[data-testid='admin-corpus-chunks-count']").TextContent.Trim());
        });
    }
    ...
    {
        // Overrides LoadAllAsync throws; stats + sources + corpus are on their happy fakes.
        var stats = Substitute.For<ICatalogStatsReadRepository>();
        stats.StreamAllManufacturersAsync(Arg.Any<CancellationToken>())
            .Returns(ci => StatsStream(ci.Arg<CancellationToken>()));
        Services.AddSingleton(stats);
        ...
        var corpus = Substitute.For<IRagCorpusStatsReader>();
        corpus.GetCorpusStatsAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new RagCorpusStats(100, [], null)));
        Services.AddSingleton(corpus);
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            // Independent loads (stats + sources + corpus) are unaffected.
            Assert.Equal("3", cut.Find("[data-testid='admin-machines-count']").TextContent.Trim());
            Assert.Equal("2", cut.Find("[data-testid='admin-sources-count']").TextContent.Trim());
            Assert.Equal("100", cut.Find("[data-testid='admin-corpus-chunks-count']").TextContent.Trim());
        });
    }

    [Fact]
    public void CorpusLoadFailure_RendersCorpusErrorSentinel_OthersUnaffected()
    {
        // Corpus throws; stats + sources + overrides are on their happy fakes.
        var stats = Substitute.For<ICatalogStatsReadRepository>();
        stats.StreamAllManufacturersAsync(Arg.Any<CancellationToken>())
            .Returns(ci => StatsStream(ci.Arg<CancellationToken>()));
        Services.AddSingleton(stats);
        ...
        var corpus = Substitute.For<IRagCorpusStatsReader>();
        corpus.GetCorpusStatsAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromException<RagCorpusStats>(
                new InvalidOperationException("AI Search unavailable")));
        Services.AddSingleton(corpus);
        ...
        cut.WaitForAssertion(() =>
        {
            cut.Find("[data-testid='admin-corpus-chunks-count-error']");
            Assert.Empty(cut.FindAll("[data-testid='admin-corpus-chunks-count']"));
            // Independent loads are unaffected.
            Assert.Equal("3", cut.Find("[data-testid='admin-machines-count']").TextContent.Trim());
            Assert.Equal("2", cut.Find("[data-testid='admin-sources-count']").TextContent.Trim());
            Assert.Equal("1", cut.Find("[data-testid='admin-link-overrides-count']").TextContent.Trim());

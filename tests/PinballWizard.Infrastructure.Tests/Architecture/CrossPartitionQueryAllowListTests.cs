...
public sealed class CrossPartitionQueryAllowListTests
{
    ...
    private static readonly Dictionary<string, string> AllowList = new(StringComparer.Ordinal)
    {
        ...
        ["MachineRepository.cs"] =
            ...
            "SearchByTitleContainsAsync SELECT TOP 25 MaxItemCount=25 — forgiving substring fallback for " +
            "getMachineByTitle, unindexed CONTAINS but miss-only and TOP-guarded); " +
            "bounded matches, metered; direct-iterator routing is tracked cleanup; " +

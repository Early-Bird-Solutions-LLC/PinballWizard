...
foreach(var key in new[]{"stern godzilla","godzilla","stern halloween","halloween"}){
  try{
    var r=await lu.ReadItemAsync<JObject>(key,new PartitionKey(key));
    var o=r.Resource;
    Console.WriteLine($"[{key}]: ids={o["opdb_ids"]?.ToString(Newtonsoft.Json.Formatting.None)} mfrs={o["manufacturers"]?.ToString(Newtonsoft.Json.Formatting.None)}");

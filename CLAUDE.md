...
Run `/local-review`. The skill spawns a `general-purpose` agent that critiques the diff across twelve categories (design, drift, error handling, security, provenance, Cosmos surface, User-Delight surface, etc.) and returns a verdict-tagged report. Fix every 🔴 finding before continuing; fix or defer-with-justification each ⚠️ finding. Skill definition: [`.claude/skills/local-review/SKILL.md`](.claude/skills/local-review/SKILL.md).
